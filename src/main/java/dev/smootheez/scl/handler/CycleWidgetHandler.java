...
import dev.smootheez.scl.widget.entry.CycleConfigEntries;
...
public class CycleWidgetHandler<E extends Enum<E>> implements WidgetHandler<E> {
     ...
     */
    @Override
    public AbstractConfigWidget createWidget(ConfigOption<E> option, @Nullable List<OrderedText> description) {
        return new CycleConfigEntries<>(Text.translatable(option.getTranslation()), description, option);
    }
}
