...
public class TextDoubleWidgetHandler implements WidgetHandler<Double> {
    @Override
    public ConfigWidgetEntry createWidget(ConfigOption<Double> option, @Nullable List<FormattedCharSequence> description) {
        return new TextDoubleWidgetEntry(Component.translatable(option.getTranslation()), description, option);
    }
}
