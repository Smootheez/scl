...
public class TextIntegerWidgetHandler implements WidgetHandler<Integer> {
    @Override
    public ConfigWidgetEntry createWidget(ConfigOption<Integer> option, @Nullable List<FormattedCharSequence> description) {
        return new TextIntegerWidgetEntry(Component.translatable(option.getTranslation()), description, option);
    }
}
